...
create table if not exists admin_response_notes (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists admin_response_notes_updated_by_idx on admin_response_notes(updated_by);
...
drop trigger if exists admin_response_notes_set_updated_at on admin_response_notes;
create trigger admin_response_notes_set_updated_at
before update on admin_response_notes
for each row execute function set_updated_at();
