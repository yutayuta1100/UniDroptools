...
export function SurveyExperience() {
  ...
  async function persistSection(completeSection: boolean) {
    ...
    if (!response.ok) {
      ...
      setPageError(payload.error ?? "保存に失敗しました。");
      return false;
    }
  ...
  function handleSubmit() {
    ...
    fetch("/api/survey/submit", {
      ...
      .catch((error) => {
        setPageError(error instanceof Error ? error.message : "送信に失敗しました。");
      })
      .finally(() => {
  ...
  return (
    <div className="space-y-8 pb-20">
      <ProgressHeader
        ...
        savedAtLabel={lastSavedAt ? `${formatDateTime(lastSavedAt)} に保存` : undefined}
      />
